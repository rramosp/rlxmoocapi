...
class Session:
    ...
    def set_grader(self, course_id, lab_id, task_id, 
        ...
        with open("grader.py", mode="w") as f:
            f.write("grader_function_name = '%s'\n\n\n%s\n"
                    % (grader_function_name, grader_source))
        
        os.system("sourcedefender encrypt grader.py")
        
        with open("grader.pye", mode="r") as f:
            encrypted_grader = f.read()
        
        os.remove("grader.pye")
        ...
        self.do_post("courses/%s/labs/%s/tasks/%s/encryptedgrader"%(course_id, lab_id, task_id), data)
    ...
    # Esteban
    def simple_get_grader(self, course_id, lab_id, task_id):
        resp = self.do_get("courses/%s/labs/%s/tasks/%s/simplegrader"%(course_id, lab_id, task_id))
        if resp.status_code==200:
            return json.loads(resp.content.decode())
    ...
    # Esteban
    def local_submit_task(self, namespace, course_id=None, session_id=None, lab_id=None, 
...
print(r)
        ...
        r = subprocess.check_output([sys.executable, 'calling_script.py'])
        grade, msg = ast.literal_eval(r.decode('utf-8'))
        
        os.remove(grader_path)
        os.remove('calling_script.py')
        
        data = {'grade': grade, 'msg': msg, 'submission_content': submission_content}
        resp = self.do_post("users/%s/courses/%s/sessions/%s/labs/%s/tasks/%s/submitgrade"%(user_id, course_id, 
                                                                                            session_id, lab_id, 
                                                                                            task_id), data)
